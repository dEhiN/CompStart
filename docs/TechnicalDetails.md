...
This script starts off the whole process. A shortcut to the Batch script is placed in the Windows Start Menu folder. The Batch script itself interactively starts the PowerShell script. It's interactive because the user is prompted if they want to run the PowerShell script. This ensures that a user could start their computer without having all their startup programs run.
