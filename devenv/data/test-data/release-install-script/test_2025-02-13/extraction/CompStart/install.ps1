...
function Install-CSFiles {
    ...
    <#
        Entry: 0
        Child Name: CompStart
        Path: Microsoft.PowerShell.Core\FileSystem::C:\Users\David\DevProj\repositories\github\CompStart\devenv\data\test-data\release-install-script\test_2025-02-12\extraction\CompStart\installer-files\CompStart
        Parent Path: Microsoft.PowerShell.Core\FileSystem::C:\Users\David\DevProj\repositories\github\CompStart\devenv\data\test-data\release-install-script\test_2025-02-12\extraction\CompStart\installer-files
        Is Container: True

        Entry: 1
        Child Name: instructions.txt
        Path: Microsoft.PowerShell.Core\FileSystem::C:\Users\David\DevProj\repositories\github\CompStart\devenv\data\test-data\release-install-script\test_2025-02-12\extraction\CompStart\installer-files\instructions.txt
        Parent Path: Microsoft.PowerShell.Core\FileSystem::C:\Users\David\DevProj\repositories\github\CompStart\devenv\data\test-data\release-install-script\test_2025-02-12\extraction\CompStart\installer-files
        Is Container: False
    #>
    ...
    foreach ($Item in $InstallerFilesList) {
        $Item.Name >> test.txt
        $Item.PSIsContainer >> test.txt
        $Item.PSParentPath >> test.txt

        $ItemFullPath = $Item.PSParentPath.Split("\")
        $ItemParentFolder = $ItemFullPath[$ItemFullPath.Length - 1]
        $ItemParentFolder >> test.txt
        "" >> test.txt
        <#
        ...
        if ($Item.PSIsContainer) {
            ...
            if (-Not (Test-Path $DestPath)) {
                Write-Host "`nCreating $Item.Name folder..." -NoNewline
                Start-Sleep $Script:SleepTime
