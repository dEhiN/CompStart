...
* Formalized the following experimental functionality into proper features:
...
* Created a PowerShell script to deploy a release:
...
* Restructured of the **CompStart** folder and its contents that constitute a release package so that the installer scripts are separated from the content to be installed.
* Reworded of the content in the `instructions.txt` file to reflect the current state of **CompStart**.
